...
### 21.8 Redesign sulle tre mosse (14 set 2026) — 25 commit `21c5002`→`7fc78f7`, in `main` dal 14 set (branch `feat/atelier-tre-mosse` mergiato ff e cancellato)
...
- **Revisione finale dell'intero branch (14 set, opus) → «with fixes», un'onda sola (`7fc78f7`)**: conteggio sfondi 53→**55** in slide, commento e handover (il branch stesso aveva aggiunto `bg-gap`/`bg-moves`); `DeckTelemetry` ricava il taglio dal pin `sessionStorage['edf-solutions-atelier:link']` quando `?s=` non c'è più (prima ogni pagina dopo la prima era loggata `full`; verificato: nuove righe `route=method` con `cut=asks`); `pnpm test` e `pnpm test:scripts` aggiunti a `.github/workflows/ci.yml`; chip di stato della slide telemetria trilingue; «Un progetto pilota / Un projet pilote»; label admin di `slide-precedent` coi nomi giusti. Re-review scoped: tutti i finding chiusi, nessuna rottura. Poi merge fast-forward in `main`, CI e Deploy verdi, URL live verificati (`/atelier/`, `/gap/`, `/moves/`, stub 200 con redirect).
