...
- **Nuova slide `capability/slide-adobe-stack` «Made with Adobe»** (non gated): Firefly Image (**53 sfondi su 4 deck**: MIM 23 · Isybank 17 · UniCredit 10 · Atelier 3), Firefly Video (**8 clip**: MIM 3 · UniCredit 2 · Isybank 2 · Atelier 1), Photoshop API (1 scontorno), credito per slide + nota onesta sul manifest C2PA perso in conversione. Conteggi letti dai `provenance.json` il 14 set: **riverificare prima di toccarli**. Registrata nel `PAGE_REGISTRY`.
...
**Minori rimandati** (dal ledger SDD, tutti in backlog §10 P2): il `cut` letto da `location.search` diventa `full` dopo la prima nav SPA (il taglio è pinnato in `sessionStorage`, va ricavato da lì) · la restrizione `anon` su `deck_events`/`deck_slide_stats` è **solo RLS** (i GRANT di progetto sono larghi) · l'occhiello della tesi «From one person to the whole sales force» è fuori tema rispetto all'h2 nuovo · rifiniture IT/FR (IT «Un pilota non è un programma» → «Un progetto pilota…»; FR «les éditeurs» nell'occhiello della cover di `gap` contro «la catégorie» della slide; nbsp e apostrofi francesi) · le etichette di stato della slide telemetria («SAMPLE DATA», «LIVE») restano in inglese nelle tre lingue.
