...
- **Pagina `apps/trenitalia-connessioni/src/pages/dossier.astro`** (route `/trenitalia-connessioni/dossier/`): war-room **interna** riservata, **orfana/fuori nav**, `noindex`, badge "Adobe internal · Riservato". Riusa il `BaseLayout` + token FS (carbonio/rosso-fs/ambra), classi **`.dw-*`** (stile locale nella pagina). 10 sezioni: executive summary · mission brief · chi-è (timeline carriera) · perimetro societario (BU Passeggeri: Trenitalia/Tper/Busitalia/FS Park — BU Merci: **FS Logistix** ex-Mercitalia Logistics, Mercitalia Rail/TX Logistik/Intermodal/Terminali Italia) · fatti verificati (badge) · big idea (Passeggeri: CartaFRECCIA personalization, MaaS multimodale — Merci/B2B: portale cargo FS Logistix, tracking MIR APP, CJA filiera) · matrice priorità P0/P1 · say/don't-say · domande+caveat · biblioteca fonti.
...
- ⚠️ **Caveat competitivo (LOCKED, il fatto più importante)**: mar 2026 il Gruppo FS ha annunciato con **Salesforce** una *foundation agentica* (Data Cloud + MuleSoft), **coordinata dalla stessa FSTechnology**, su 840M+ passeggeri + logistica merci → Salesforce presidia già lo strato CDP/CX. **Niente pitch CDP greenfield**: posizionare Adobe come complementare (AEM/GenStudio/Firefly content layer, CJA analytics, Target) o sfidante mirato. Naming: usare **FS Logistix** (non "Mercitalia Logistics"); carriera/cifre LinkedIn (~45 AMS, ~160 app) **non verificate** → contesto, non dato Adobe; il ramo Merci potrebbe non essere nel mandato DBP (da confermare a voce).
