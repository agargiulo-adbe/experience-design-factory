...
- **Alfabeti — Ministero dell'Istruzione e del Merito (MIM)** (`apps/mim-alfabeti`, **2–3 set 2026**) · IT default + toggle EN, palette light istituzionale carta/blu + **Titillium Web**. **Ri-architettato customer-facing** (`acab255`): **7 route** pulite (spina = comunicazione al personale B1), zero contenuto interno; la vecchia sezione «realtà» + tutti i caveat/competitor spostati nel **dossier interno `/dossier/?t=<token>`** (RLS + secret-link, IT/EN + PDF, 16 sezioni, contenuto solo su Supabase; seed riservato git-ignored). Fonti cliccabili su ogni slide. `audit:deck` 0 hard. Registrata in console (`0010`). Vedi **§28**.
...
|---|---|---|
| **Alfabeti (MIM)** | **Ri-architettata customer-facing** (2–3 set 2026, `acab255`): deck **7 route** pulite (era 8), IT default + toggle EN, palette light carta/blu + Titillium Web. Contenuto interno spostato nel **dossier `/dossier/?t=<token>`** (RLS+secret-link, IT/EN+PDF, 16 sez.). Fonti cliccabili. `audit:deck` 0 hard. | Vedi §28. Base `/mim-alfabeti/`. **Seedata in console** (`0010`). Seed dossier riservato git-ignored (`docs/Ministero dell'Istruzione/0011_…sql`). |
...
> voci P0/P1 nella task list di sessione (effimera). Aggiornato 2026-09-03.
