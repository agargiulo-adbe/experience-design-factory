...
- **Alfabeti — Ministero dell'Istruzione e del Merito (MIM)** (`apps/mim-alfabeti`, **2 set 2026**) · IT default + toggle EN, palette light istituzionale carta/blu + **Titillium Web**. 8 sezioni; la sezione **«realtà»** è gated dalla soluzione *interno* (**doppio uso** client/interno). `audit:deck` 0 hard. Base `/mim-alfabeti/`. Dossier strategico bilingue in `docs/` (git-ignored). Vedi **§28**.
...
|---|---|---|
| **Alfabeti (MIM)** | **Nuova** (2 set 2026): deck 8 sezioni, IT default + toggle EN, palette light carta/blu + Titillium Web. Sezione «realtà» gated da soluzione *interno* (doppio uso client/interno). `audit:deck` 0 hard. | Vedi §28. Base `/mim-alfabeti/`. Console Supabase NON seedata (P2). Dossier strategico bilingue in `docs/` (git-ignored). |
...
> voci P0/P1 nella task list di sessione (effimera). Aggiornato 2026-09-02.

**P1 — prossimo**
- **[P1] Dossier Attribution UniCredit — verifica live** — area: `apps/unicredit-engagement/src/pages/dossier.astro` · la migration **`0008_restricted_docs` è stata applicata al DB remoto** (2 set, SQL editor); **restano da verificare in browser** il render autenticato, il toggle IT/EN, il download PDF e i link LinkedIn (il browser tool era bloccato da una sessione Chrome). QC live dopo login super admin (`agargiulo@adobe.com`) → apri `/unicredit-engagement/dossier/`. Vedi §27 (agg. 2026-09-02).
...
- **[P1] Accesso da VPN** — **azione UMANA** · le esperienze deployate (GitHub Pages, repo pubblico) sono raggiungibili dalla rete ufficio Adobe ma **bloccate su VPN** (categoria proxy/**Zscaler** su `*.github.io`). Ticket IT aperto (17 lug 2026); **stato riscontro da verificare** → follow-up sul ticket. Workaround demo = screen-share. Piano B (opt-in, non fatto) = redeploy su **Vercel** (agg. 2026-09-01).
...
- **[P2] Alfabeti (MIM) — seed console Supabase** — area: `supabase/` · manca la migration seed del registry `experiences` per la card in Super Admin (come Eni). Registrata invece in deploy.yml/factory-hub/deck-audit (agg. 2026-09-02).
