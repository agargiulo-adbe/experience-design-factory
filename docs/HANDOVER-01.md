...
pnpm --filter @agargiulo-adbe/experience-core test   # 40 test del blocco scoping (cost-model/scenario/scenario-store, Vitest)
...
| **Ferrari Racing** | Stabile + **sezione /scoping** (calcolatore licensing: **15 lug** il modello Collaboration è stato riscritto per **replicare 1:1 il workbook Adobe** — burn ufficiali, funnel matched→impressions/conversions, modalità detailed/simple/direct, pacchetto crediti consigliato; CJA invariato. Save resiliente. **40 test**, vedi §14 + §18). Product Mockup (Genstudio/Rtcdp/MockupSlide); CJAMockup/ExpressMockup pending. `audit:deck` 0 (74→0). | Bilingue EN default. `prevHref` su tutte le pagine. |
