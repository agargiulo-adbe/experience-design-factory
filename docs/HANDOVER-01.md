...
- **Supabase** — migrations applicate al DB remoto: `0001`–`0003` + `0004_scenarios.sql` + `0005_hub_registry.sql` + `0006_seed_agos.sql` + `0007_seed_atelier.sql` (Atelier `live`) + **`0008_restricted_docs.sql`** (tabella `restricted_docs` + RLS + seed Dossier Attribution UniCredit; applicata 2 set; §27) + **`0009_restricted_doc_share.sql`** (colonna `share_token` + RPC `get_shared_doc` SECURITY DEFINER per l'unlisted secret-link del dossier; applicata 2 set via `supabase db query --linked`; §27.7). Se ricrei il progetto, riesegui in ordine (`supabase/README.md`). Memoria `super-admin-console`.
