...
**P1 — prossimo**
- **[P1] Principi «Selling to Executives» → skill di intake + showcase** — area: `skills/experience-brief/SKILL.md` (+ `apps/factory-showcase` weaving/citazione + rigenerazione artefatti `public/skill/*`) · **richiesto dall'utente, in pausa allo step di approvazione design** (l'utente ha deviato sul task Atelier prima di confermare). Fonte: workbook `docs/Adobe Material/Selling to Executives/Selling to Executives Adobe Workbook_Feb 10-11 2026.pptx` (Jacques Sciammas, 163 slide). Design concordato: (1) **ristrutturazione sostanziale** di `SKILL.md` — executive rilevante + 3 lenti (Financial/Operational/Growth), obiettivo/pain prima del prodotto, ROI rule (prova ≥1 di revenue/cost/risk), narrativa before→after, output = **value-proposition brief**; (2) sito showcase **solo weaving** in `#flow`/`#skill`/`#what` (no nuova sezione) **con citazione esplicita** del metodo; (3) rigenerare `.zip`+`.md` in `public/skill/`. Ripartire chiedendo conferma del design (agg. 2026-09-01).
